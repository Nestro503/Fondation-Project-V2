...
# Load the image
image_path = "images/diapo4.png"  # Spécifiez le chemin de votre image
...
def main():
    pygame.init()  # Initialisez Pygame

    # Calculez le nombre de rangées en fonction du ratio entre la largeur et la hauteur de l'image
    ROW_SCALE_FACTOR = 0.01 * 10  # Facteur d'échelle pour ajuster le nombre de rangées
    ...
    WIN = pygame.display.set_mode((WIDTH, HEIGHT))  # Définissez la surface d'affichage
    pygame.display.set_caption("A* Path Finding Algorithm")  # Définissez le titre de la fenêtre
